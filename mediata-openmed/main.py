...
app = FastAPI(
    ...
    description=(
        ...
        "with a text normalisation fallback.  Value descriptions are generated by a small "
        "instruction-tuned LLM (Qwen/Qwen2.5-0.5B-Instruct)."
    ),
    version="1.0.0",
)
...
DESCRIBE_MODEL_NAME = "Qwen/Qwen2.5-0.5B-Instruct"

_describe_pipeline = None   # set to (tokenizer, model) tuple on success, or "fallback"

_DESCRIBE_SYSTEM_PROMPT = (
    "You are a clinical data annotator. "
    "Given a clinical phrase about a measurement or category, "
    "write a SHORT clinical description (2-6 words). "
    "Output ONLY the description, no punctuation at the end, no explanations."
)


def _load_describe_model_once():
    """Load the generative LLM used to generate value descriptions.
    ...
    try:
        ...
        mdl = AutoModelForCausalLM.from_pretrained(
            DESCRIBE_MODEL_NAME, dtype=torch.float32
        )
        mdl.eval()
...
def _generate_desc(prompt: str, pipe) -> str:
    ...
    The *prompt* is the full contextual phrase built by ``_build_value_phrase``
    or ``_build_category_phrase``.  It is passed to the instruction-tuned
    generative LLM which produces a short clinical description.
    ...
    prompt="score 10 of 10 measuring Ability to use toilet"
      → "High ability to use toilet"  (or similar)

    prompt="Ischemic as a category of Stroke"
      → "Ischemic stroke"

    prompt="Male as a category of Gender"
      → "Male gender"
    """
    ...
    try:
        ...
        # Trim to the first sentence in case the model generates more than asked.
        result = re.split(r"[.\n]", result)[0].strip()
